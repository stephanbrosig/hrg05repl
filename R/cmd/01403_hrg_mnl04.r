#  HRG05 344ff mnl w dummies f nonlinear effects 
...
asm <- readRDS(file = "dat/01401_hrg_imp.Rds")  %>% 
...
database <- database %>% 
                     ...
                     mutate(choice_newn =   as.numeric(factor(as.vector(choice_new), levels = 1:6, 
                                                         labels = c("CART","CARNT","TR","BW","LR","BUS")))) %>%
                     mutate(fare_BW1 = ifelse(fare_BW == 1, 1, 0),
                            fare_BW3 = ifelse(fare_BW == 3, 1, 0),
                            fare_BW5 = ifelse(fare_BW == 5, 1, 0))  # create fare dummies   
...
### Set core controls
apollo_control = list(
  modelName       = "01403_hrg_mnl4",
  modelDescr      = "01403a_hrg_mnl4: Replication of hrg p. 344 dummy coding (nonlinear)",
  indivID         = "id", 
  outputDirectory = "01403_hrg_mnl", 
  panelData = TRUE
#  workInLogs=TRUE
)
...
apollo_beta <- c(                                                                            
 ...
 b_fare_bw1    = -.04,
 b_fare_bw3    = -.04,
 b_fare_bw5    = 0,   # not necessary but may help interpretation of output
 asc_lr    = 0        # not necessary but may help interpretation of output
 )                       
...
apollo_fixed <- c("b_fare_bw5","asc_lr") # c() # 
...
V[["BW"]]    <- asc_bw     + b_fare_bw1 * fare_BW1 + b_fare_bw3 * fare_BW3 + b_fare_bw5 * fare_BW5
...
##  LL1 <- readRDS(file = "01403_hrg_mnl1/01403_hrg_mnl1_model.rds")$finalLL
##  
##  str(LL1)
##  
##  LL2 <- model$finalLL
