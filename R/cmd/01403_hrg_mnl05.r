...
#asm$ndrivlic  <- as.numeric(asm$drivlic) # to simulate erroneous use of drivlic instead of ndrivlic
#asm$ndrivlic[is.na(asm$ndrivlic)] <- 0  # same
...
#                                                         labels = c("CART","CARNT","TR","BW","LR","BUS")))  %>%
                     ...
                     mutate(vehlic = ndrivlic * numbvehs )  # create interactions
...
### Set core controls
apollo_control = list(
  modelName       = "01403_hrg_mnl5",
  modelDescr      = "01403a_hrg_mnl5: hrg 2005 p. 352: interactions1 (lin)",
  indivID         = "id", 
